...
class TikTokAnalyzer:
    ...
    def filter_videos(self, videos):
        ...
        for video in videos:
            if (
                ...
                and video.get("play_count", 0) > 0
                and video.get("digg_count", 0) > 0
            ):
    ...
    def transcribe_video(self, audio_path):
        try:
            ...
            result = self.whisper_model.transcribe(
                audio_data, language=None, task="transcribe", fp16=False
            )
    ...
    async def process_videos(self, query, max_videos=5):
        ...
        for video in filtered_videos:
            try:
                ...
                print(
                    f"Error processing video {video.get('video_id', 'unknown')}: {str(e)}"
                )
                continue
    ...
    async def analyze_videos(self, query, max_videos=5):
        self.masterquery = query
        print(f"\nAnalyzing TikTok videos about: {query}")
        return await self.process_videos(query, max_videos)
